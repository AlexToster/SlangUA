...
> Кожен ключ AI-провайдера — це пул: змінна приймає перелік ключів через кому, і адаптер обертається на наступний ключ, коли поточний вичерпав ліміт або квоту. Саме це робить безкоштовні тарифи придатними для production. Перед тим як складати кілька ключів в один пул, варто перевірити умови провайдера: деякі забороняють мати кілька безкоштовних акаунтів, і ротація цього не виправляє.
...
| `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `OPENROUTER_API_KEY` | — | Ключі AI-провайдерів (опційні; потрібен хоча б один провайдер). Кожна змінна приймає **перелік ключів через кому** — пул, у якому ключ із вичерпаним лімітом відкладається, а запит обслуговує наступний. |
| `AI_KEY_COOLDOWN_RATE_MS` | `60000` | На скільки відкладається ключ, який упав у ліміт запитів. Значення навмисно невелике: Gemini повідомляє про вичерпану добу безкоштовного тарифу так само, як про ліміт на хвилину. |
| `AI_KEY_COOLDOWN_QUOTA_MS` | `3600000` | Те саме для ключа з явно вичерпаною квотою. |
| `AI_KEY_COOLDOWN_INVALID_MS` | `3600000` | Те саме для ключа, який провайдер відхилив як недійсний. Такий ключ додатково логується на рівні `error`: виправити його може лише людина. |
