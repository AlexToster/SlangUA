...
describe('Translate Integration Tests', () => {
  ...
  describe('POST /translate (direct persistent)', () => {
    it('should return 401 for missing JWT', async () => {
      const response = await app.inject({
        method: 'POST',
    ...
    it('should successfully translate and persist for GEN_Z style', async () => {
      const response = await app.inject({
        method: 'POST',
        url: '/api/v1/translate',
        headers: { authorization: `Bearer ${accessToken}` },
    ...
    it('should apply separate rate limit for persistent translate', async () => {
      // The persistent translate has its own rate limit (10/min in test config)
      // This test verifies it's independent from preview/save limits
      for (let i = 0; i < 10; i++) {
        const response = await app.inject({
          method: 'POST',
          url: '/api/v1/translate',
          headers: { authorization: `Bearer ${accessToken}` },
          payload: { text: `Direct translate ${i}`, style: 'GEN_Z' },
        });
        // Should succeed (test limit is 10/min)
        expect(response.statusCode).toBe(200);
      }
      ...
      const response = await app.inject({
        ...
        url: '/api/v1/translate',
        headers: { authorization: `Bearer ${accessToken}` },
        payload: { text: 'Direct translate rate limited', style: 'GEN_Z' },
      });
      expect(response.statusCode).toBe(429);
    });
  });
  ...
  describe('Rate limit independence', () => {
    it('should have independent rate limits for preview, save, and translate', async () => {
      ...
      // And direct translate should still work (different limit)
      const translateResponse = await app.inject({
        method: 'POST',
        url: '/api/v1/translate',
        headers: { authorization: `Bearer ${accessToken}` },
        payload: { text: 'Direct translate still works', style: 'GEN_Z' },
      });
      expect(translateResponse.statusCode).toBe(200);
    });
  });
});