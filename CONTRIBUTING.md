...
**Документація (корінь проєкту), якщо змінювали README:**

```bash
node scripts/check-readme-parity.mjs   # без залежностей: два README мусять лишатися дзеркалом
```

Ті самі команди виконує CI ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)) на кожен push і PR у `main`: окрема job для backend (з `npx prisma generate` перед перевірками і Docker для integration-тестів), окрема для frontend і окрема `Docs` без `npm ci` взагалі. Локальний прогін перед PR все одно потрібен — CI ловить те, що ви забули, а не замінює перевірку.
...
- **Українською:** `README.md`, `AGENTS.md`, `CONTRIBUTING.md`, [`docs/operations.md`](docs/operations.md).
- **Англійською:** технічна документація в `plans/**` (architecture, ROADMAP, API, style engine, briefing) і довідник змінних [`docs/configuration.md`](docs/configuration.md).
...
### README — дзеркальна пара

README існує у двох мовах: [`README.md`](README.md) українською (сторінка проєкту на GitHub) і [`README.en.md`](README.en.md) англійською. Це **дзеркало**, а не окремий документ: ті самі розділи в тому самому порядку, ті самі зображення й бейджі. Зміна в одному файлі без другого — незавершена зміна.

Два правила, які легко порушити:

1. **Приклади стилів не перекладаються.** Вивід — це і є продукт; жарт, який пережив переклад англійською, демонструє вже не те. У `README.en.md` кожен зразок у «лапках» побайтово той самий, що в `README.md`, а англійському читачеві призначена окрема колонка з поясненням механізму — що саме стиль зробив із текстом.
2. **Посилання на українські документи в англійському README мають позначку `(in Ukrainian)`.** Інакше читач іде за посиланням, очікуючи англійську.

Перевіряє це [`scripts/check-readme-parity.mjs`](scripts/check-readme-parity.mjs) — окрема job `Docs` у CI, без залежностей:

```bash
node scripts/check-readme-parity.mjs
```

Скрипт звіряє склад і порядок розділів `##`, побайтову тотожність зразків, наявність перемикача мов в обидва боки, а також те, що всі посилання, анкори й зображення розв'язуються і що ASCII-схема архітектури лишилася вирівняною по колонках. Дрейф двох README нічого не ламає — англійська сторінка просто починає тихо брехати, тому перевірка автоматична.
