...
Документація, якщо чіпав README:

```bash
node scripts/check-readme-parity.mjs   # дзеркало README.md ↔ README.en.md; залежностей не потребує
```
...
- Кожна нова змінна середовища додається у Zod-схему в [`src/config/index.ts`](src/config/index.ts), у `.env.example` **і** описується в довіднику [`docs/configuration.md`](docs/configuration.md) (англійською). Невалідна конфігурація навмисно зупиняє процес на старті.
- Виняток — змінні, які читає не застосунок, а **Docker Compose** (`POSTGRES_PASSWORD`, `BACKUP_*`) і Vite (`VITE_*`). У Zod-схемі їх немає й бути не повинно: схема описує те, що потрібно процесу для запуску, і додавання туди пароля Postgres зробило б локальний запуск проти наявної бази неможливим. Живуть вони в окремому розділі в кінці `.env.example` та в [`docs/configuration.md`](docs/configuration.md), поруч із поясненням, чому їх там немає.
...
- Українською: `README.md`, `AGENTS.md`, `CONTRIBUTING.md`, `docs/operations.md` — продуктовий і командний контекст.
- Англійською: технічні документи в `plans/**` (architecture, ROADMAP, API, style engine, briefing) і довідник змінних `docs/configuration.md`.
...
- **README — дзеркальна пара.** `README.md` (українська, сторінка на GitHub) і `README.en.md` — один документ двома мовами: ті самі розділи в тому самому порядку, ті самі зображення й бейджі. Правка одного файлу без другого — незавершена правка. Приклади стилів у `README.en.md` **не перекладаються** і побайтово збігаються з українськими: вивід — це і є продукт, а англійському читачеві призначена окрема колонка з поясненням механізму. Посилання на українські документи в англійському README позначаються `(in Ukrainian)`. Звіряє все це `node scripts/check-readme-parity.mjs` — окрема job `Docs` у CI, без залежностей.
...
6. Ілюстрація — `frontend/src/assets/styles/<name>.webp` (де `<name>` — id стилю в нижньому регістрі з `_`→`-`, напр. `gen-z`) плюс статичний `import` і запис у `STYLE_ART` в `frontend/src/utils/styleArt.ts`. `STYLE_ART` — вичерпний `Record<SlangStyle, string>`, тож відсутня ілюстрація ламає typecheck, доки її не додано. Формат нового файлу — **240×180 WebP (4:3), ~10–13 КБ**: це розмір плитки в сітці стилів, і атрибути `width`/`height` у `StyleDropdown.tsx` мусять дорівнювати розміру файлу, інакше картинка резервує не свій бокс до декодування.
...
Провайдер розпізнавання мови змінюється так само й теж без коду: `SttService` розмовляє OpenAI-сумісним `/v1/audio/transcriptions`, тому достатньо `STT_BASE_URL`, `STT_MODEL` і `STT_API_KEY` (кілька ключів через кому). Це **не** інстанс AI-ланцюга: у нього власний `KeyPool` з id `stt`, власні кулдауни `STT_KEY_COOLDOWN_*` і власний лімітер — вичерпана квота транскрипції не має паркувати ключ, потрібний перекладачеві. Порожній `STT_API_KEY` = голосового вводу в цьому деплої не існує (`voiceInputAvailable: false`, кнопки мікрофона немає). Довідник змінних — [`docs/configuration.md`](docs/configuration.md#voice-input-stt) (англійською).
