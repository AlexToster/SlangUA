...
- **9.4 — Monitoring, reduced to configuration** `[next]`
...
- **9.5 — Deployment runbook** `[planned]`
  Extend [docs/operations.md](../docs/operations.md) with the full sequence: certificate issuance and renewal with certbot, `__SERVER_NAME__` substitution, the first `.env` on the server (including the deployment-only variables), the first admin, upgrades, and rollback. Rollback needs saying out loud rather than implying: rolling the image back is one command, rolling the *schema* back is not — `prisma migrate deploy` has no down step, so a release that migrated the database can only be undone by restoring a dump. Getting that wrong under pressure is how the data is lost. Server-specific scripts stay outside the repository as before ([10-repository-hygiene.md](docs/10-repository-hygiene.md)); the procedure does not have to.
...
- **9.7 — Public deployment** `[planned]`
  Domain, TLS via certbot, the HTTPS template in place of the bootstrap one, `.env` with real secrets, `CORS_ALLOWED_ORIGINS` and `TRUST_PROXY` set for the real origin, and the Mini App URL registered in BotFather. Plus the monitoring that 9.4 reduced to configuration: the UptimeRobot check on `/health/ready` with Telegram notifications, and the backup heartbeat.
- **9.8 — Pre-flight on the real deployment** `[planned]`
  ...
  **The slang-quality matrix per provider is *not* narrowed.** It is the product rather than a checklist item: the fallback chain can hand a user a visibly worse translation from the second provider without anything failing or logging, and that is only ever found by reading output. Every configured provider, every style.
  ...
  Plus one case the deployment adds by itself: on Telegram Web the Mini App runs in a cross-site iframe, where a `SameSite=Lax` refresh cookie is not sent, so the session recovers through a full reload instead of a silent refresh. Verify it on Telegram Web and decide deliberately whether that is acceptable or whether the cookie needs `SameSite=None`.
