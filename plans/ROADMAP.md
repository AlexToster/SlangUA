...
**Current state, 2026-09-03: the MVP is complete, and the rollout to the production network is in progress.** Stages 1–8 are closed and every deliverable inside the MVP scope is in place — the architecture, the data model and the Prisma schema, the API, the backend, the Style Engine with its six styles, the frontend and the Telegram Mini App, the admin panel and Ukrainian speech-to-text (both built out of order at the owner's request), and the automated suites. What remains is [Stage 9](#stage-9--deployment-in-progress), where [9.1–9.5](#stage-9--deployment-in-progress) are done, [9.6](#stage-9--deployment-in-progress) was dropped, the public deployment at [9.7](#stage-9--deployment-in-progress) is under way, and the pre-flight pass and sign-off follow it.

"MVP complete" is a claim about scope and not about operability, and the distinction is the whole point of Stage 9 still being open: the four things [9.9](#stage-9--deployment-in-progress) requires — a restore that has been performed, an alert that has fired, logs that stop growing, and a runbook somebody else could follow — are what separate a deployment that runs from one somebody else can rely on. Until they have happened, the stage stays `[in progress]` however finished the feature set looks.
...
- **9.7 — Public deployment** `[in progress]`
  ...
  **This is the active item.** Which of those steps have been carried out is not something the repository can answer — none of them leaves a trace in it, by design: the domain, the certificate, the real `.env` and the BotFather registration all live on the server or in a third-party console. So this item does not get sub-states here; it closes when [9.8](#stage-9--deployment-in-progress) can begin, which is to say when the Mini App loads from the public HTTPS origin.
- **9.8 — Pre-flight on the real deployment** `[next]`
