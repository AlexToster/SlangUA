...
**The MVP is complete as of 2026-09-03, and the rollout to the production network is in progress.** Read that as a statement about scope rather than about operability: every deliverable inside the MVP is in place, and what is still open is Stage 9 — the deployment itself and the proof that it can be operated.
...
**Stage 9 — deployment — is in progress**, ordered from the irreversible failures outward. Done: Postgres backups (a `db-backup` service running `scripts/backup-postgres.sh`, one dump a day, 7 daily + 4 Sunday copies, with the restore procedure in `docs/operations.md`), reverse-proxy correctness (`client_max_body_size` and `proxy_read_timeout`, whose nginx defaults would each have broken a healthy application, plus immutable caching for the hashed assets and `no-store` for `index.html`), log retention caps (a `logging:` block per service, ~170 MB across the stack, because a full disk breaks Postgres writes and truncates the dump that would have saved the situation), monitoring reduced to configuration (an exact-match `location = /health/ready` in both nginx templates, since a monitor pointed at the SPA origin is green forever, plus `BACKUP_HEARTBEAT_URL` as a dead-man's switch for dumps that quietly stop), and the deployment runbook in `docs/operations.md`. Dropped: `docker build` in CI (9.6) — the deploy itself already fails before recreating anything, so it buys a nicer failure location, not safety; revisit in Stage 10. In progress: **the public deployment (9.7)** — domain, TLS, the HTTPS template, real secrets, BotFather, UptimeRobot; none of it leaves a trace in the repository, so its per-step state is not tracked here. Remaining after it: the pre-flight pass on the real deployment (9.8 — the Android and iOS on-device checks, the slang-quality matrix per provider, the Telegram Web cookie case) and sign-off (9.9), where "operable" means a restore that has been performed, an alert that has fired, logs that stop growing and a runbook somebody else could follow. Stage 10 collects post-MVP ideas: speech-to-text (built out of order, see below), OCR, premium tier, analytics.
...
**Out of order, at the owner's request (2): Ukrainian speech-to-text**, and deliberately in a narrower form than the Stage 10 bullet names — an **input method** on Translate, not "voice messages". Five steps, all five done: 1 the endpoint (`POST /transcribe` with its own `stt` key pool, an OpenAI-compatible provider behind `STT_BASE_URL`/`STT_MODEL`, its own per-minute budget, a container allowlist and a size ceiling); 2 the recording mechanics (`useAudioRecorder`, a 30-second cap, per-platform container detection, the microphone released on stop); 3 the microphone button in the editor footer, in the slot the Paste button held, appending the transcript to the end of the draft; 4 documentation and scope. A fifth step then replaced the recording state itself: the filled red square read as a fault, so the pill now expands into a recording row with a blinking dot, a count-up timer and a live level meter, and gains a cancel chip. Nothing in the preview/save contract changed and no audio is stored anywhere. Real-time dictation, TTS, voice commands and speaker recognition remain out of scope. See §7 (Voice input), §10 and §16.
...
- There is **no `POST /translate`**. The one-shot translate-and-persist endpoint was removed in Stage 8: it had no client, duplicated the preview route's validation, age gate and prompt-injection path, and was the only way to persist a translation the user had never seen. Preview then save is the whole contract — do not reintroduce a shortcut past it.
...
- **`GET /user/me`** — JWT. Returns the profile including `ageConfirmedAdult` and two server-computed capability flags: `isAdmin` and `voiceInputAvailable` (true only when the deployment configured `STT_API_KEY`). Both are hints about what the client should draw, never a decision about access.
...
- **`POST /transcribe`** — JWT. Body `{ audio, mimeType }`: `audio` is base64 of the recorded container with no `data:` prefix, `mimeType` is the recorder's own type with codec parameters (1–120 chars). Returns `{ text, model }` — the trimmed transcript and the model that produced it. Errors: 400 `VALIDATION_ERROR` / `STT_EMPTY_AUDIO`; 401; 413 `STT_AUDIO_TOO_LARGE`; 415 `STT_UNSUPPORTED_AUDIO_TYPE`; 422 `STT_NO_SPEECH`; 429 `RATE_LIMIT_EXCEEDED` (this endpoint's own 6/min per user) and 429 `STT_QUOTA_EXCEEDED` (every key in the STT pool parked, with `Retry-After` **and** a `retryAfter` field in seconds); 503 `STT_UNAVAILABLE` (no `STT_API_KEY` in this deployment), `STT_FAILED`, `RATE_LIMITER_UNAVAILABLE`.
- Base64 in JSON rather than `multipart/form-data`, because `@fastify/multipart` is not a dependency and a 30-second Opus capture fits in a JSON body. The route carries its **own** `bodyLimit` derived from `STT_MAX_AUDIO_BYTES` plus base64 expansion, so an over-long capture is refused before the body is buffered.
- `mimeType` is trusted as a format signal rather than sniffed, because the container differs by platform (Android Chromium `audio/webm;codecs=opus`, iOS WKWebView `audio/mp4`) and it is what maps to the extension the upstream upload needs. Anything outside the allowlist (`webm`, `ogg`, `opus`, `mp4`, `mpeg`, `wav`, `x-wav`, `x-m4a`, `flac`) is refused, so arbitrary bytes never reach a paid provider.
- Three properties make this an input method rather than a second translation path. It is **an input method**: the endpoint returns text, the client appends it to the draft, and it becomes a `Translation` row only through the ordinary preview/save, so nothing in that contract changed. It has **its own keys** — `STT_API_KEY` is a comma-separated list behind a `KeyPool` with pool id `stt`, deliberately not an AI pool, because a spent transcription quota must not park a key the translator still needs on the same account. And **no audio is ever persisted**: the decoded buffer lives inside the handler and reaches no store — not Postgres, not Redis, not a temp file, not a log line — which `test/integration/transcribe.integration.test.ts` asserts rather than leaving to documentation. The age gate does not apply here: it belongs to the styles that produce restricted output, and transcription produces none.
...
- **`GET /admin/metrics`** — same auth. Read-only usage figures: `{ generatedAt, retentionDays, totalUsers, perMinute: { minutes, series: [{ startedAt, requests, errors }] }, last24h: { hours, requests, errors, users, series: [{ startedAt, requests, errors }] }, daily: [{ date, requests, errors, users, averagePerUser }], topUsers: [{ userId, requests }] }`. The minute series is oldest first and always exactly `METRICS_MINUTE_SERIES_LENGTH` long with idle minutes present as zeros, so a graph cannot silently compress them; `daily` is newest first (today is `daily[0]`, no date arithmetic on the client) with UTC day boundaries; `topUsers` is today's list, descending, capped at `METRICS_TOP_USERS_LIMIT`. `last24h` is a **rolling** window of 24 hourly buckets, oldest first, ending with the hour in progress — it exists because `daily[0]` says almost nothing at 01:00 UTC; its `users` is an exact distinct count over the whole window (`SUNION` over the hourly sets, not a sum of them, which would count a person once per active hour). `totalUsers` is the only figure here that is not from Redis: a Postgres `COUNT` over `User`, because bucket-derived totals expire and an account count that falls during a quiet week would read as data loss. `errors` means `statusCode >= 500` — a `400`, `401` or `429` is the API working as designed. `userId` is the **internal** numeric id rendered as a string: never a Telegram id, never a username, because the panel needs to tell heavy users apart, not to identify them. Not counted at all: `OPTIONS` preflights, `/health*` and `/api/v1/admin/*` itself. Errors: 401 `ADMIN_SESSION_REQUIRED` / `ADMIN_SESSION_INVALID`; 404; 429; 503.
...
Rate limits are separate Redis key prefixes per concern: `ratelimit:global` (100/min per IP, all routes except `/health`), plus `auth`, `refresh`, `preview` (12/min), `save` (10/min), `share` (10/min), `history`, `user`, `styles`, `stt` (6/min per user, a budget of its own because every call spends shared upstream transcription quota), `webhook` (30/min), `admin-login` (5 per 5 min) and `admin` (120/min). Every response carries `X-RateLimit-Limit/Remaining/Reset`; a 429 also carries `Retry-After`.
...
**Voice input keeps nothing.** `POST /transcribe` is authenticated for a reason that is not symmetry with the rest of the API: an open transcription endpoint is a free STT proxy billed to the operator. On top of the JWT it carries its own per-user budget (6/min), because the quota it spends is the deployment's and not the caller's, and it refuses anything outside a container allowlist and above `STT_MAX_AUDIO_BYTES` before the body is buffered. **The audio is never persisted anywhere** — not Postgres, not Redis, not a temp file, not a log line — and neither is the transcript: the endpoint answers with text and forgets it, and that text becomes a stored row only through the ordinary preview/save, under the same validation, prompt-injection check and age gate as anything typed. The transcription keys are a separate pool (`STT_API_KEY`, pool id `stt`) with their own cooldowns, so an exhausted transcription quota cannot park a key the translator needs; keys never reach the client, which is the whole reason the recording goes through us instead of straight to the provider.
...
Three screens, exactly three bottom-nav items, no separate Home screen — the root *is* Translate, even though its tab is drawn as a house: the bar is three unlabelled lucide glyphs (house `/`, clock `/history`, cog `/settings`) with no separators, and the accessible names live on in visually hidden text. Mobile-first from 320 px. The nav is a translucent glass bar positioned absolutely inside the non-scrolling shell so the page passes under it (`backdrop-filter` needs something to blur), and `.app-layout-content` is padded by the bar's height plus the safe area, so the nav still never hides the result area or the Telegram safe area. The one deliberate exception is the open style grid, which runs under the glass and pads its own bottom by the measured overlap so the last row of tiles can still be scrolled clear. There is a fourth route, `/admin`, deliberately outside the navigation: it is the only lazily loaded page, its entry point appears in Settings only when `/user/me` returns `isAdmin: true`, the step-up token lives in memory and is attached as `X-Admin-Token` to `/admin/*` requests and nothing else, and for everyone else the panel does not exist in the API either. The page holds three independent sections — the provider chain with its kill-switch, the usage metrics, and the error feed — each with its own query, its own loading state and its own retry, so an unreachable feed does not blank the providers an operator opened the panel to switch off. A `401` from any of them means the step-up session expired and re-locks the panel; on admin paths `404`, not `401`, is the retriable status.
...
**Input rules.** Multiline textarea capped at 200 px with auto-resize; a grapheme counter (`123 / 1 000`) that warns from 850 and hard-stops at 1000 without silent truncation; and "Випадкова фраза" which inserts one of ~500 locally generated Ukrainian phrases, never repeating the immediately previous one, and never creating History. The footer's second slot now holds the **microphone**, which took the place of the Paste button (whose behaviour — Clipboard API only on explicit tap, degrading to "Встав текст вручну" on denial — stays documented in `plans/docs/08-frontend-design.md` rather than in the code, pending a decision on whether it returns).

**Voice input is an input method.** The mic is icon-only at rest, 44×44 at every width and filled with the same blue plate as "Поділитись" (it is the only control in the action row that starts an action of its own), and exists at all only when `GET /user/me` reported `voiceInputAvailable: true` **and** the WebView actually exposes `MediaRecorder` — absence rather than a disabled control, because a button that can never work is worse than no button. While recording, that same button becomes the recording row: the pill grows across the action row (`flex-grow` 0 → 0.8 — four fifths of the free space, the rest absorbed by the right chip's auto margin; no measuring and no JS animation) and holds a blinking dot, a count-**up** timer in seconds only (`03` — the cap is half a minute, so minutes never occur), and a live level meter; the space comes from "Випадкова фраза", which collapses to its dice icon in the same moment, and the right-hand 44×44 chip becomes "Скасувати запис" instead of "Очистити". The plate returns to paper here, and red is the border, the dot and a thin limit rule along the bottom of the pill — never a red fill, which was the first version and read as a fault. The meter reads the real microphone level through Web Audio and is best-effort in three directions (no `AudioContext`, no `requestAnimationFrame`, `prefers-reduced-motion`): it is the only thing that distinguishes "recording" from "permission granted but hearing nothing", and it stays out of React's render path — a function plus refs writing CSS custom properties at ~15 fps, so the editor is not re-rendered fifteen times a second. Four states (idle, requesting permission, recording, transcribing — the pill stays expanded through the request, at full ink, so the row does not snap back and forth twice per interaction) with a 30-second hard cap, a 0.5-second floor below which nothing is sent, and both the microphone track and the meter's `AudioContext` closed on every exit. Cancelling is not a quieter stop: the clip is not sent at all and the user gets neither text nor an error. The recognized text is **appended to the end** of the draft in one `setDraftText`, so it triggers exactly one debounce and one preview; if the merged text would exceed 1000 graphemes the append is refused outright with a toast, never truncated. "Випадкова фраза" is disabled while a recording or a request is in flight. The layering is three-deep on purpose: `useAudioRecorder` knows nothing about the API, `useVoiceInput` composes it with `POST /transcribe` and owns the user-facing copy per error code (distinguishing this deployment's limiter, which the user can wait out, from the provider quota, which they cannot), and `TranslatePage` keeps no capture state at all.
...
**Settings** covers appearance (theme `Як у Telegram` / `Світла` / `Темна`), interaction (haptics on by default, sound off by default), translation and age gate (default style, the 18+ self-attestation dialog), support and about, and logout. **Persistence is split on purpose:** `defaultSlangStyle` and `ageConfirmedAdult` live server-side via `GET/PATCH /user/me`; theme override, sound and haptics live in this Mini App's `localStorage` — explicitly not Telegram CloudStorage. Settings is also the **only** writer of `defaultSlangStyle`: switching style on Translate applies to that session and nothing else, so the saved default cannot be changed by trying a style out.
...
Speech-to-text, deliberately separate from the AI block: `STT_API_KEY` (empty by default — and empty means voice input does not exist in this deployment, `voiceInputAvailable: false` and no button, rather than a microphone that answers 503; accepts a comma-separated pool read by its own `KeyPool` with id `stt`), `STT_BASE_URL` (`https://api.groq.com/openai/v1`), `STT_MODEL` (`whisper-large-v3-turbo`), `STT_LANGUAGE` (`uk`), `STT_TIMEOUT_MS` (30000), `STT_MAX_AUDIO_BYTES` (1048576, which also derives the route's own `bodyLimit`), `STT_KEY_COOLDOWN_RATE_MS` (60000), `STT_KEY_COOLDOWN_QUOTA_MS` / `STT_KEY_COOLDOWN_INVALID_MS` (3600000), and `STT_RATE_LIMIT_WINDOW_MS` / `STT_RATE_LIMIT_MAX_REQUESTS` / `STT_RATE_LIMIT_KEY_PREFIX` (60000 / 6 / `ratelimit:stt`). Any OpenAI-compatible `/v1/audio/transcriptions` endpoint is a two-variable swap, not a code change.
...
Frontend component tests live next to their components as `src/**/*.test.tsx` and cover `StyleDropdown` (2-D grid keyboard navigation — Left/Right move by one and Up/Down by the column count, both wrapping; `aria-activedescendant`; locked styles routed to the age-gate handler), `ConfirmDialog`, `BottomNav`, `Toast` (including the five-second auto-dismiss under fake timers) and `ErrorBanner` (per-code retry labels). jsdom implements no layout, so `src/test/setup.ts` stubs `Element.prototype.scrollIntoView`.
...
Voice input has its own integration suite (`test/integration/transcribe.integration.test.ts`), and its central assertion is a negative one: after a successful transcription **nothing** exists in Postgres or Redis that holds the audio or the transcript. Around that: the transcript returned for an authenticated caller, the Android container uploaded as `.webm` and the iOS one as `.mp4`, the upstream request carrying a key from the `stt` pool, an unauthenticated request refused *before* any upstream call, an unnameable container and oversized audio and non-base64 bodies refused, silence reported as `STT_NO_SPEECH`, every key in the pool tried before `STT_QUOTA_EXCEEDED`, an upstream fault answered as `STT_FAILED` after a single attempt (transcription is not retried — the clip is already gone from the user's hands), the endpoint's own rate limit, and `voiceInputAvailable` appearing on the profile. On the client, `frontend/src/hooks/useAudioRecorder.test.ts` and `useVoiceInput.test.ts` run against a `MediaRecorder` mock with fake timers — including a capture in a WebView with no Web Audio at all (the clip still arrives, the level simply reads 0) and one where a stubbed `AudioContext` proves the meter is opened over the same stream and closed with the microphone — `TextInput.test.tsx` covers the recording row (elapsed seconds, the meter, the collapsed "Випадкова фраза", cancel replacing clear), and `utils/audio.test.ts` covers the chunked base64 encoding.
...
1. Age gate is enforced on the server, once and for real: `TranslationService` compares the style's `ageRestricted` against `User.ageConfirmedAdult` before the preview-cache lookup and before any provider call. `GET /styles` does not filter, and the locked tile in the UI is cosmetic — never treat it as the second gate.
...
14. The operator kill-switch is never merged into the circuit breaker and never expires. It lives in the Redis hash `ai:provider:disabled` with no TTL, is read once per request *before* the breakers, excludes a provider from both the fallback chain and the recovery probe, and is cleared only by a human. It also outranks the preview cache: `TranslationService` checks `aiService.hasPermittedProviders()` before the cache lookup on both translate paths, so with everything switched off a warm entry answers `503 AI_PROVIDER_UNAVAILABLE` instead of serving the dead provider's own output for the rest of the TTL. Breaker state is not consulted there — a cached answer should still survive a real outage. Reading it fails closed; presence of the field is the switch even when the value is unparseable.
...
16. Voice-input audio is never persisted — not Postgres, not Redis, not a temp file, not a log line — and neither is the transcript. `POST /transcribe` stays authenticated (an open one is a free STT proxy on the operator's bill) and keeps its own per-user budget, because the quota it spends belongs to the deployment. Transcription keys stay a separate pool (`STT_API_KEY`, id `stt`) so a spent transcription quota cannot park a translator's key, and voice stays an input method: the transcript enters the draft and becomes a row only through the ordinary preview/save, never a translation path of its own.
...
2. The warm-cache branch of the one-shot `translate()` no longer 500s on the `Translation.previewId` unique constraint — `P2002` was handled on that path too. Superseded in Stage 8: the method and its endpoint were removed, so the branch no longer exists.
...
6. Both `/translate*` endpoints share one status→reason-phrase table, so 404/409/410 no longer serialize as `Internal Server Error`; a 409 from `POST /translate/save` now returns the already-saved translation so the client can recover it.
...
A second remediation pass, split into six groups. Both passes are committed — the warning that used to stand here, that all of it lived in the working tree only, is out of date.

- **Dead client code removed.** `translateDirect` (no caller; the endpoint it called was removed in Stage 8 — see §7), `showMainButton`/`hideMainButton` and the equally unused `showBackButton`/`hideBackButton`. `MainButton` typings in `telegram.d.ts` were kept: they describe the host SDK, not a product promise. `MainButton` and deep links were dropped from ROADMAP Stage 7 with the reason recorded in place, so the decisions are not re-litigated.
...
- **Health and hygiene.** `GET /health/ready` with an integration test and §10 of `plans/docs/04-api.md`; an `api` healthcheck in `docker-compose.production.yml`; the `Dockerfile` pinned through one global `ARG NODE_IMAGE`; `.gitattributes` enforcing `eol=lf`; `test-rate-limit.ps1` deleted as superseded by `test/integration/rate-limit.integration.test.ts`.

**Not yet verified on Windows.** The sandbox runs `tsc` and the Style Engine smoke test and nothing else: `vitest` cannot start there (`Cannot find native binding` for rolldown, because `node_modules` was installed on Windows, and the npm registry answers 403), Docker is absent, and `oxlint` is a native binary too. So `npm run test:unit`, `npm run test:integration`, `npm test`, and in `frontend/` `npm test`, `npm run lint` and `npm run build` are all owner-side. Test files that have been type-checked but never executed by a runner: `test/unit/metrics.test.ts`, `test/unit/error-feed.test.ts`, `test/integration/admin-metrics.integration.test.ts`, `test/integration/admin-errors.integration.test.ts`, and from Stage 8 `test/integration/flow.integration.test.ts`, `test/integration/security.integration.test.ts`, `test/unit/prompt-injection.test.ts`, `frontend/src/App.test.tsx`, `frontend/src/services/api.test.ts`, `frontend/src/pages/TranslatePage.test.tsx`.
...
| Voice input, end to end | `src/services/stt/stt.service.ts`, `src/routes/transcribe.ts`, then `frontend/src/hooks/useAudioRecorder.ts` and `useVoiceInput.ts`, and the recording row in `frontend/src/components/TextInput.tsx` with `VoiceMeter.tsx` |
