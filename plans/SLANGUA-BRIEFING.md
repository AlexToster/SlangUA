...
**Voice input is an input method.** The mic is icon-only, 44×44 at every width, and exists at all only when `GET /user/me` reported `voiceInputAvailable: true` **and** the WebView actually exposes `MediaRecorder` — absence rather than a disabled control, because a button that can never work is worse than no button. Four states (idle, requesting permission, recording with a corner countdown, transcribing) with a 30-second hard cap, a 0.5-second floor below which nothing is sent, and the microphone track stopped on every exit. The recognized text is **appended to the end** of the draft in one `setDraftText`, so it triggers exactly one debounce and one preview; if the merged text would exceed 1000 graphemes the append is refused outright with a toast, never truncated. "Випадкова фраза" is disabled while a recording or a request is in flight. The layering is three-deep on purpose: `useAudioRecorder` knows nothing about the API, `useVoiceInput` composes it with `POST /transcribe` and owns the user-facing copy per error code (distinguishing this deployment's limiter, which the user can wait out, from the provider quota, which they cannot), and `TranslatePage` keeps no capture state at all.
...
| Voice input, end to end | `src/services/stt/stt.service.ts`, `src/routes/transcribe.ts`, then `frontend/src/hooks/useAudioRecorder.ts` and `useVoiceInput.ts` |
