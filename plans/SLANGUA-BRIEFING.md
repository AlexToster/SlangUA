...
**`AIService`** adds a per-instance circuit breaker: failures accumulate, the breaker opens at `CIRCUIT_BREAKER_FAILURE_THRESHOLD`, and goes half-open after `CIRCUIT_BREAKER_RESET_MS`. `translate()` walks eligible providers sequentially; if every breaker is open it retries the whole chain as a last resort. An instance whose keys are all on cooldown is skipped without counting as a provider failure.
