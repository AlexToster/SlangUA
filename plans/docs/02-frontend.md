...
- **`pages/`** — three routed screens (`TranslatePage`, `HistoryPage`, `SettingsPage`) wrapped by `AppLayout`, which provides the single-scroll shell and the bottom navigation. The nav is a translucent glass bar positioned absolutely inside the shell (not in the flow, and deliberately not `fixed`), so the page scrolls under it; `.app-layout-content` carries a matching `padding-bottom` so nothing ends up hidden behind it. The open style grid is the one element allowed to run under the glass: its height is not clamped to the bar, and the overlapped part is compensated by an inner `padding-bottom` measured from the bar itself.
...
- **`hooks/`** — `useAudioRecorder` wraps `MediaRecorder`/`getUserMedia` and hands back one finished clip (or one classified failure) per capture, and `useVoiceInput` composes it with `POST /transcribe` into the single control object `TextInput` renders. The split is deliberate: the recorder knows nothing about the API, and neither knows anything about the draft.
- **`utils/`** — `localSettings` (theme and default-style persistence), `previewAttempts` (client-side regeneration accounting), `styleLabels` / `providerLabels` (display names), `text` (grapheme-aware counting), `audio` (base64 encoding of a recorded clip, walked in chunks because the spread form overflows the call stack on a real capture).
