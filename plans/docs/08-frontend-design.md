...
> Уточнення після голосового вводу: у рядку дій на місці «Вставити» тепер стоїть **мікрофон**. Сама «Вставити» прибрана з інтерфейсу, але не з проєкту: опис вище лишається чинним описом кнопки, `readTextFromClipboard` і його тести на місці, і рішення — вертати кнопку п'ятим контролом чи прибрати назовсім — відкрите. Мікрофон — тільки піктограма, коло 44×44 у будь-якій ширині: четвертий підпис у рядок не вміщався навіть на 360px, а кнопка, що згортається до іконки лише на вузьких екранах, дала б два різні інтерфейси для однієї дії. Лічильник секунд, що лишилися, — значок у куті кнопки, тому власної ширини в рядку він не займає (саме це й дозволило додати контрол, не забравши місце в «Випадкової фрази»).
>
> Кнопки немає взагалі — не вимкненої, а відсутньої, — коли `GET /user/me` повернув `voiceInputAvailable: false` або цей WebView не вміє записувати (`MediaRecorder`/`getUserMedia` недоступні). Мікрофон, який завжди відповідає `503`, гірший за його відсутність. Стани: спокій (`Mic`), очікування дозволу, запис (`Square`, заливка кольором «зупинити», пульсація по зсуву фарби, лічильник) і розпізнавання (спінер, кнопка вимкнена). Кожен стан має власний `aria-label`, а сама кнопка — `aria-pressed`, бо це перемикач; лічильник `aria-hidden`, щоб не диктувати секунди в `aria-live`.
>
> Один дотик починає запис, другий — завершує; стеля — **30 секунд**, після яких запис зупиняється сам. Коротший за пів секунди дотик не летить на сервер, а просить сказати фразу. Розпізнаний текст **дописується в кінець чернетки** (не замінює її й не питає підтвердження) одним `setDraftText`, тому наявний debounce у 900 мс дає рівно один preview, а не по одному на слово. Якщо разом із наявним текстом вийшло б понад 1 000 графем, транскрипт не вставляється: чернетка лишається як була, а користувач бачить toast — тихо обрізати надиктоване не можна. «Випадкова фраза» вимкнена, поки триває запис або розпізнавання. Аудіо не лишається й на клієнті: доріжки зупиняються одразу після завершення запису, тому системний індикатор мікрофона гасне.
>
