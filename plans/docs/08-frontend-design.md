...
>
> Під час запису та сама кнопка стає **рядком запису**: пігулка розтягується на весь вільний простір рядка й тримає блимаючу точку, відлік і шкалу гучності. Місце вона забирає не в лічильника і не в правого чипа, а в «Випадкової фрази», що в ту саму мить згортається до кубиків (підпис іде в нульову ширину, тому кнопка звужується, а не перестрибує). Розгортання — один `flex-grow` 0 → 1 у CSS: він інтерполюється як число, тому ні вимірювань, ні JS-анімації тут немає.
>
...
>
> Червоне в записі — це контур пігулки, точка й лінійка ліміту, **а не залита кнопка**: залитої плашки в цьому оформленні немає ніде, і саме вона читалася як помилка. Відлік іде **вгору й тільки в секундах** («03»): стеля — півхвилини, тому хвилин тут не буває, а «скільки вже сказано» корисніше за дедлайн. Скільки з ліміту витрачено, показує тонка лінійка вздовж низу пігулки (`--rec-progress` у `::after`), а не друге число. Точка блимає сходинками (`steps(1, end)`), бо плавного світіння в різографі немає. Шкала гучності — чотирнадцять смужок, що читають реальний рівень із мікрофона через Web Audio: єдине, що відрізняє «пише» від «дозвіл дали, але нічого не чути». Рівень віддається функцією й пишеться в CSS-змінні кадром ~15 fps, тому редактор не перемальовується п'ятнадцять разів на секунду; за відсутності `AudioContext`, `requestAnimationFrame` або за `prefers-reduced-motion` смужки просто стоять на місці, а запис від цього не залежить.
>
