...
- Порожній стан: один рядок про те, що переклад з'явиться автоматично, набраний тим самим форматом, що placeholder редактора (розмір `--font-size-placeholder`, вага 500, підказковий колір), — порожній результат читається як пара до порожнього поля, а не як повідомлення над ним. `--font-size-placeholder` на крок менший за `--font-size-editor` (18px замість 20px, на телефоні 16 замість 18) і живе в `styles/global.css` одним токеном на обидва порожні стани: підказка не мусить важити стільки ж, скільки набраний текст, який її заміняє, але між собою поле й результат мусять важити однаково. Другого рядка з підказкою «почніть вводити текст» тут немає: він дублював placeholder. Підказка про мінімум трьох символів лишається тільки тоді, коли текст уже набирають, але його ще замало для перекладу.
- Завантаження: «Перекладаємо…» для першого результату; «Оновлюємо…» поверх попереднього результату для наступних.
- Успіх: перекладений текст, назва використаного стилю, кнопка **Копіювати** та, коли Telegram inline sharing готовий і результат eligible, кнопка **Надіслати в Telegram**. Назва стилю в підвалі — просто підпис: без плашки й без контуру, лише велика літера, вага і повне чорнило проти приглушеної назви моделі поруч. Пігулкою вона бути перестала, бо повторювала кнопку стилю над відповіддю, а плашка без контуру в різографі читається як брак фарби, тому знімається все разом.
...
- **Розділ «Додаток» — дві однакові кнопки: «Про додаток» і «Github».** Підпису в рядку немає: «Про додаток» ліворуч нічого не додавав до кнопки «Детальніше» праворуч, тому сам став кнопкою, а на її місце стало посилання на GitHub автора. Раніше воно жило дією всередині тоста «Про додаток» і зникало разом із повідомленням; тепер це кнопка розділу й доступна завжди. Обидві дії другорядні, тож обидві кнопки — та сама пігулка `--face` (`settings-btn-secondary`) і рівні половини рядка. Посилання відкривається через `openExternalLink`, а не `<a href>`: у Mini App звичайне посилання лишилось би у вебвʼю Telegram. Тост «Про додаток» тепер показує один рядок — `SlangUA v<версія> перекладач українських стилів 2026`, де версія приходить з `__APP_VERSION__` (з `package.json` під час збірки), — і власної дії більше не має.
