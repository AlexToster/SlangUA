...
SlangUA — розважальний, а не філологічний сервіс із вираженим WOW-ефектом: максимально трансформувати форму тексту, повністю зберігаючи його ключові факти та намір. Стиль має бути миттєво впізнаваним. Гіпербола, перебудова фрази й характерна подача дозволені; вигадування нової події, ролі, насильства, технічного інциденту чи юридичного факту — ні.

Стилі **не рівносильні за ступенем трансформації** — це навмисно:

| Стиль | Характер трансформації |
|---|---|
| `KANCLER` | Навмисно бюрократичний, роздуває текст приблизно у 2–2,5 рази |
...
| `GALICIAN` | Тепла львівсько-галицька ґвара з батярською подачею |

Мета кожного стилю: власний Voice, приклади трансформацій, характерний словник, список забороненої лексики — так, щоб стилі були максимально контрастними один до одного. Контраст не означає механічно замінювати кожне слово: промпт задає, коли стилістична рамка доречна, а лексикон дає матеріал для виразної, але зрозумілої трансформації.
...
- **Transformation Rules** — наскільки сильно й як саме трансформується текст (для `KANCLER` тут явно фіксується "2–2,5× довжини"; для `GEN_Z` — "довжина приблизно зберігається")
...
Кожен стиль має щонайменше 3 приклади, включно з демонстрацією характерної структурної трансформації, а не лише заміни окремих слів. Приклади — частина дієвого промпту: loader додає їх після лексиконів, тому вони мусять відповідати актуальним `prompt.md` і `lexicon.json`, не містити заборонених слів та не вигадувати фактів. Подальше зростання: 3 → 50 → 100 → 300, поступово, не зараз.
...
**Жорсткий guardrail:** `preferred` — **20–40 найхарактерніших слів**, не звалище. Лексикон застосовується до кожного з шести стилів і має бути водночас виразним та семантично придатним: рідкісні, багатозначні або контекстно вузькі слова пояснюються в `prompt.md`, а слова одного стилю не підмінюють голос іншого.
...
*Пізніше уточнення (STREET `1.0.3`):* із цієї таблиці для STREET залишилися `базарювати`, `братва`, `авторитет` — іменник `базар` виведено з `forbidden`, обґрунтування в §9.2. Не повертати його, спираючись на цю таблицю: вона фіксує стан міграції, а не поточний словник.
...
- **id (registry):** `galician`; **enum (Prisma/API):** `GALICIAN`; **version:** `1.0.1`; **ageRestricted:** `false`.
- **Характер:** львівсько-галицька ґвара, а не молодіжний сленг. Тепла, дещо церемонна інтонація з батярським шармом; германо-польські запозичення, здрібнілі форми та характерна граматика. «Фест» працює як підсилювач, «нівроку» — як схвалення, а «ся» й «най» не додаються механічно до кожного дієслова.
...
- **id (registry):** `pofeni`; **enum (Prisma/API):** `POFENI`; **version:** `1.1.1`; **ageRestricted:** `true`; **шаринг:** дозволений лише користувачам із `ageConfirmedAdult: true`; для непідтверджених — `403 AGE_RESTRICTED_SHARE` на `POST /share/inline` (див. [09-telegram-sharing.md](09-telegram-sharing.md) §3).
...
- **Регістр після звільнення:** окрема, легко втрачувана риса стилю. Людина, що вийшла, говорить так і про побут — роботу, гроші, родину, — а не лише про зону. Для розважальної виразності переклад може переносити побутову сцену в кримінально забарвлену рамку або згадувати тюремний побут, але не вигадує конкретного злочину, насильства, імені чи події.
- **Смислові групи лексики:** побут (`шконка`, `барак`, `кича`, `шизо`, `пайка`), ієрархія (`пахан`, `авторитет`, `блатний`, `мужик`, `фраєр`, `пасажир`, `стукач`, `шнир`), адміністрація (`вертухай`, `кум`), комунікація (`малява`, `прогон`, `ґрєв`), економіка (`общак`, `лаве`, `зелень`, `капуста`) і «поняття» як нормативна система: «по поняттях», «за базар відповідати», «рамси не попутати», «зашквар». Арготизми не стають приводом переходити на російський синтаксис.
...
- **Розмежування зі STREET:** `хата` і `бабло` належать STREET і стоять у `forbidden` у `pofeni/lexicon.json`; натомість `базарювати`, `братва`, `авторитет` стоять у `forbidden` у `street/lexicon.json`. Іменник `базар` — свідомий виняток: із версії STREET `1.0.3` він **не** заборонений, бо це водночас звичайне українське слово «ринок» (саме в цьому значенні `README.md` називає базар одним із джерел голосу STREET), а арготичне значення «розмова» тримають дієслова — `базарювати` у STREET-овому `forbidden`, `базарити` у POFENI-вому `preferred` — і конструкція «за базар відповідати». До версії `1.1.0` `prompt.md` стилю POFENI сам радив «дім → хата», «гроші → бабло», тобто інструктував використати слова, які той самий system prompt нижче забороняв блоком `Avoid these words:`. Поточне правило не підставляє тюремний побут механічно: стиль може криміналізувати рамку й згадати шконку, барак чи пайку, але зберігає основну дію оригіналу.
- **Версія:** після узгодженого оновлення `prompt.md`, `lexicon.json` і `examples.json` версія піднята `1.1.0` → `1.1.1`. Це обовʼязково: `styleVersion` входить у HMAC-ключ кешу превʼю, і без підняття версії теплий кеш віддавав би результати старого промпту.
