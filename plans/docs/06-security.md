...
- The password is stored only as a scrypt hash in `ADMIN_PASSWORD_HASH` (`N=16384, r=8, p=1`, 16-byte salt, 32-byte key, `maxmem = 256·N·r`, compared with `timingSafeEqual`). The operator generates it locally with `scripts/hash-admin-password.mjs`, which reads the password from stdin — never from argv, which would put it in the shell history and the process list — enforces a 12-character minimum, and prints no plaintext. The hash format is validated by shape at boot, so a mangled copy-paste fails the start instead of looking like a permanently wrong password. In a deployed `.env` the value has to be single-quoted: it always carries three `$`, and Docker Compose interpolates the values it reads through `env_file`, so an unquoted line loses `$N` and `$p` to substitution and the shape check rejects a hash that is in fact correct. The schema also refuses to boot with an allowlist and no hash, which would be single-factor access.
...
**The switch outranks the preview cache too.** `TranslationService` reads it before consulting the HMAC-keyed preview cache, on both the preview and the direct-translate path, for the same reason the age gate and the injection check run there: a warm entry must not become a way around a decision a human made. Without that, killing the last provider mid-incident would keep serving that provider's own cached output — for whoever happened to have a matching entry, for the rest of the preview TTL — and the operator would have no way to tell whether the flip had taken effect. Circuit-breaker state is deliberately not consulted at that point: a breaker heals itself, and through a real outage a cached answer is exactly what should still be served. Only the human decision closes the cache.
...
**The metrics carry no identity at all beyond an internal number.** The per-hour and per-day user sets are keyed by the internal `User.id`; the response returns a bare count for the rolling window and a string id in the top-users list. Never a Telegram id, never a username, never text: the panel needs to tell heavy users apart, not to identify them. Day boundaries are UTC so that a row does not change value when a server's timezone does. The all-time account total is a `COUNT` over `User` — a number of rows, so it carries no identity by construction.
